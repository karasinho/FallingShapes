import { Application, Graphics, Rectangle, Container, Sprite, RenderTexture, Assets, } from 'pixi.js';
...
export async function initPixiApp(canvas_wr) {
...
export class ShapeFactory {
    ...
    drawPolygon({ g, sides, radius = this.engine.settings.circle_radius, fillColor = this.engine.currentColors[g.kind ?? 'triangle'], strokeColor = ShapeFactory.getRandomColor(), strokeWidth = 2, }) {
        if (sides < 3) {
...
export class ShapePool {
    ...
    getRandomKind() {
        const SHAPE_KINDS = ['triangle', 'quad', 'pentagon', 'hexagon', 'circle', 'ellipse', 'blob', 'tyan'];
        return SHAPE_KINDS[Math.floor(Math.random() * SHAPE_KINDS.length)];
    }
    async spawnShape(x, y, animate) {
...
async function initEngine(canvas_wr) {
...
function initTicker(engine) {
    ...
    engine.app.ticker.add((ticker) => {
        ...
        while (spawnAcc >= spawnInterval) {
            ...
            const area = engine.spawnArea.hitArea;
